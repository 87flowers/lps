...
namespace lps::doubling {
  ...
  LPS_INLINE constexpr basic_bit_mask<T, N, Env>::basic_bit_mask(detail::bit_mask_base_t<N> value) {
  ...
  [[nodiscard]] LPS_INLINE constexpr std::array<T, N> basic_bit_mask<T, N, Env>::to_array() const {
    const std::array<std::array<T, N / 2>, 2> result {
      raw[0].to_array(),
      raw[1].to_array(),
    };
    return std::bit_cast<std::array<T, N>>(result);
  }
  ...
  template<class T, usize N, class Env>
  [[nodiscard]] LPS_INLINE constexpr basic_bit_mask<T, N, Env>::vector_type basic_bit_mask<T, N, Env>::to_vector() const {
    return std::bit_cast<vector_type>(to_array());
  }
